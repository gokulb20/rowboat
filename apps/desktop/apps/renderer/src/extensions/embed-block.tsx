...
import { blocks } from '@crewm8/shared'
...
function getEmbedUrl(provider: string, url: string): string | null {
  ...
  if (provider === 'figma') {
    // Convert www.figma.com/design/:key/... → embed.figma.com/design/:key?embed-host=crewm8
    const figmaMatch = url.match(/figma\.com\/(design|board|proto)\/([\w-]+)/)
    if (figmaMatch) {
      return `https://embed.figma.com/${figmaMatch[1]}/${figmaMatch[2]}?embed-host=crewm8`
    }
    // Legacy /file/ URLs
    const legacyMatch = url.match(/figma\.com\/file\/([\w-]+)/)
    if (legacyMatch) {
      return `https://embed.figma.com/design/${legacyMatch[1]}?embed-host=crewm8`
    }
  }
  return null
}
