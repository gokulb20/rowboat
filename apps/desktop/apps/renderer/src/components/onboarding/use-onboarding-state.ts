...
export type OnboardingPath = 'composio' | 'byok' | null
...
export function useOnboardingState(open: boolean, onComplete: () => void) {
  ...
  // Composio/Gmail state (used when signed in with Composio account)
  ...
  // Auto-advance from Composio connection step when OAuth completes
  useEffect(() => {
  ...
  // Switch to composio path from BYOK inline callout
