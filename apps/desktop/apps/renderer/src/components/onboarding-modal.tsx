...
type OnboardingPath = 'composio' | 'byok' | null
...
export function OnboardingModal({ open, onComplete }: OnboardingModalProps) {
  ...
  // Auto-advance from Composio connect-in step when OAuth completes
  useEffect(() => {
    if (onboardingPath !== 'composio' || currentStep !== 0) return

    const cleanup = window.ipc.on('oauth:didConnect', (event) => {
      if (event.provider === 'composio' && event.success) {
  ...
  const renderSignInStep = () => {
    const rowboatState = providerStates['composio'] || { isConnected: false, isLoading: false, isConnecting: false }
      ...
      <div className="flex flex-col items-center text-center">
        ...
        {rowboatState.isConnected ? (
          ...
          <div className="flex flex-col items-center gap-4 w-full max-w-xs">
            <Button
              onClick={() => {
                setOnboardingPath('composio')
                startConnect('composio')
              }}
              size="lg"
              className="w-full"
          ...
          <>
            ...
            {(useComposioForGoogle || useComposioForGoogleCalendar) && (
              <div className="space-y-2">
                ...
                {useComposioForGoogle && renderGmailRow()}
