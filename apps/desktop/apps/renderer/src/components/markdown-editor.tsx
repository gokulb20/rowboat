...
import { Crewm8MentionPopover } from './crewm8-mention-popover'
import '@/styles/editor.css'

type Crewm8MentionMatch = {
...
export const MarkdownEditor = forwardRef<MarkdownEditorHandle, MarkdownEditorProps>(function MarkdownEditor({
  ...
  const [activeCrewM8Mention, setActiveCrewM8Mention] = useState<Crewm8MentionMatch | null>(null)
  ...
  const updateCrewM8MentionState = useCallback(() => {
    if (!editor) return
    const { selection } = editor.state
    if (!selection.empty) {
      setActiveCrewM8Mention(null)
    ...
    // Match @crewm8 at a word boundary (preceded by nothing or whitespace)
    const match = textBefore.match(/(^|\s)@crewm8$/)
    if (!match) {
      setActiveCrewM8Mention(null)
      setRowboatAnchorTop(null)
      return
    ...
    const triggerStart = textBefore.length - '@crewm8'.length
    const from = selection.from - (textBefore.length - triggerStart)
    const to = selection.from
    setActiveCrewM8Mention({ range: { from, to } })
  ...
  const updateAtMentionState = useCallback(() => {
    ...
    // If the full "@crewm8" is already typed, let updateCrewM8MentionState handle it
    if (query === 'rowboat') {
      setActiveAtMention(null)
      setAtAnchorPosition(null)
      return
  ...
  useEffect(() => {
    if (!editor) return
    editor.on('update', updateCrewM8MentionState)
    editor.on('selectionUpdate', updateCrewM8MentionState)
    return () => {
      editor.off('update', updateCrewM8MentionState)
      editor.off('selectionUpdate', updateCrewM8MentionState)
    }
  }, [editor, updateCrewM8MentionState])
  ...
  const handleRowboatAdd = useCallback(async (instruction: string) => {
    ...
    if (activeCrewM8Mention) {
      // Insert a temporary processing block
      const blockData: Record<string, unknown> = { instruction, processing: true }

      const insertFrom = activeCrewM8Mention.range.from
      const insertTo = activeCrewM8Mention.range.to
  ...
  }, [editor, activeCrewM8Mention, rowboatBlockEdit, frontmatter, onFrontmatterChange, notePath])
  ...
  const atMentionOptions = useMemo(() => [
    { value: 'rowboat', label: '@crewm8', description: 'Research, schedule, or run tasks with AI' },
  ], [])
  ...
  const handleSelectAtMention = useCallback((value: string) => {
    ...
    if (value === 'rowboat') {
      // Replace "@<partial>" with "@crewm8" — this triggers updateCrewM8MentionState
      editor
        .chain()
        .focus()
        .insertContentAt(
  ...
  return (
    <div className="tiptap-editor" onKeyDown={handleKeyDown}>
      ...
      <div className="editor-content-wrapper" ref={wrapperRef} onScroll={handleScroll}>
        ...
        <Crewm8MentionPopover
          open={Boolean((activeCrewM8Mention || rowboatBlockEdit) && rowboatAnchorTop)}
