...
import { TrackBlockSchema, type TrackSchedule } from '@crewm8/shared/dist/track-block.js'
...
export function TrackModal() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('crewm8:open-track-modal', handler as EventListener)
    return () => window.removeEventListener('crewm8:open-track-modal', handler as EventListener)
  ...
  const handleEditWithCopilot = useCallback(() => {
    if (!detail) return
    window.dispatchEvent(new CustomEvent('crewm8:open-copilot-edit-track', {
      detail: {
        trackId: detail.trackId,
