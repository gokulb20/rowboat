...
import * as composioClient from '@crewm8/core/dist/composio/client.js';
import { composioAccountsRepo } from '@crewm8/core/dist/composio/repo.js';
import { invalidateCopilotInstructionsCache } from '@crewm8/core/dist/application/assistant/instructions.js';
import { CURATED_TOOLKIT_SLUGS } from '@crewm8/shared/dist/composio.js';
import type { LocalConnectedAccount, Toolkit } from '@crewm8/core/dist/composio/types.js';
import { triggerSync as triggerGmailSync } from '@crewm8/core/dist/knowledge/sync_gmail.js';
import { triggerSync as triggerCalendarSync } from '@crewm8/core/dist/knowledge/sync_calendar.js';

const REDIRECT_URI = 'http://localhost:8081/oauth/callback';

/**
 * Create a single-shot HTTP server on the given port that resolves
 * when the OAuth callback URL is requested.
 */
function createCallbackServer(port: number): Promise<http.Server> {
    return new Promise((resolve, reject) => {
        const server = http.createServer((_req, res) => {
            res.writeHead(200, { 'Content-Type': 'text/html' });
            res.end('<html><body><p>OAuth complete! You can close this window.</p></body></html>');
        });
        server.listen(port, () => {
            resolve(server);
        });
...
export async function initiateConnection(toolkitSlug: string): Promise<{
    ...
    try {
        ...
        const server = await createCallbackServer(8081);
