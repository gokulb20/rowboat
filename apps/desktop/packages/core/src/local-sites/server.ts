...
const IFRAME_HEIGHT_MESSAGE = 'crewm8:iframe-height';
...
(() => {
  const SITE_CHANGED_MESSAGE = '__CREWM8_SITE_CHANGED_MESSAGE__';
  const SITE_EVENTS_PATH = '__CREWM8_SITE_EVENTS_PATH__';
  ...
  const MESSAGE_TYPE = '__CREWM8_IFRAME_HEIGHT_MESSAGE__';
...
function injectIframeAutosizeBootstrap(html: string): string {
  const bootstrap = IFRAME_AUTOSIZE_BOOTSTRAP
    .replace('__CREWM8_IFRAME_HEIGHT_MESSAGE__', IFRAME_HEIGHT_MESSAGE)
    .replace('__CREWM8_SITE_CHANGED_MESSAGE__', SITE_RELOAD_MESSAGE)
    .replace('__CREWM8_SITE_EVENTS_PATH__', SITE_EVENTS_PATH)
  if (/<\/body>/i.test(html)) {
...
<body>
  <div class="edit-bar">
    <button class="edit-btn" onclick="window.location.href='crewm8://md-edit?path='+encodeURIComponent('${escapeHtml(filePath)}')">Edit Source</button>
  </div>
  ${html}
</body>
</html>`;
