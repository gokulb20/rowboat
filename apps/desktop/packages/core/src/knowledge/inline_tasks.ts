...
import { inlineTask } from '@crewm8/shared';
 ...
 * Also supports legacy @crewm8 format.
 */
function parseBlockContent(contentLines: string[]): { instruction: string; schedule: InlineTaskSchedule | null; lastRunAt: string | null; targetId: string | null } | null {
    ...
    try {
        ...
        // Legacy format: @crewm8 lines + optional schedule: JSON line
    }

    // Legacy fallback: parse @crewm8 instruction and schedule: line
    ...
    const firstRowboatLine = instructionLines.find(l => l.trim().startsWith('@crewm8'));
    const rawInstruction = firstRowboatLine?.trim() ?? instructionLines.join('\n').trim();
    const instruction = rawInstruction.replace(/^@crewm8:?\s*/, '');
...
/**
 * Process a @crewm8 instruction via the inline task agent.
...
export async function processCrewInstruction(
    instruction: string,
    noteContent: string,
    notePath: string,
): Promise<{
    ...
    const message = [
        `Process the following @crewm8 instruction from the note "${notePath}":`,
        '',
